...
const PlaylistDetail = ({ playlistId, userId, onStartRoom, onBack, onPlay }) => {
  ...
  const fetchPlaylist = async () => {
    const { data } = await axios.get(`http://127.0.0.1:5001/api/playlists/${playlistId}`, {
  ...
  const searchAndAdd = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        `http://127.0.0.1:5001/api/search?q=${encodeURIComponent(searchQ.trim())}&mode=smart`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteTrack = async (e, mediaId) => {
    e.stopPropagation();
    try {
      const { data } = await axios.delete(`http://127.0.0.1:5001/api/playlists/${playlistId}/media/${mediaId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setPlaylist(data);
    } catch (err) {
  ...
  const uploadFile = async (e) => {
    ...
    try {
      const { data } = await axios.post(`http://127.0.0.1:5001/api/playlists/${playlistId}/upload`, formData, {
