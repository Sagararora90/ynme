...
const PlaylistList = ({ onSelect }) => {
  ...
  const generateAiPlaylist = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post((import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`) + '/api/ai/generate', { prompt: name }, { headers: { Authorization: `Bearer ${token}` } });
      const res = await axios.post((import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`) + '/api/playlists', { name: `AI: ${name}`, mediaItems: data.items }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const createPlaylist = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post((import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`) + '/api/playlists', { name, isPublic: false }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const joinPlaylist = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`}/api/playlists/join/${joinCode}`, {}, { headers: { Authorization: `Bearer ${token}` } });
