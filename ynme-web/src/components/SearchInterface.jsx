...
const SearchInterface = ({ socket, userId, onPlay, onAddToPlaylist }) => {
  ...
  useEffect(() => {
    if (history.length > 0) {
      ...
      axios.get(`${import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`}/api/search/related?title=${encodeURIComponent(last.title)}&id=${last.id || ''}&platform=${last.platform || ''}`, {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        `${import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`}/api/search?q=${encodeURIComponent(query.trim())}&mode=${mode}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const fetchPlaylists = async () => {
    try {
      const { data } = await axios.get((import.meta.env.VITE_API_URL || `http://${window.location.hostname || 'localhost'}:5001`) + '/api/playlists', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setPlaylists(data);
    } catch {}
